...
create_function(
	...
	////////////////////////////////////////////////////////////////////////////
	// 
	if (_type.compare("DirichletBoundary_KoecherBruchhaeuser1") == 0) {
		AssertThrow(
			options.size() == 2,
			dealii::ExcMessage(
				"dirichlet_boundary options invalid, "
		...
		function = std::make_shared< diffusion::dirichlet_boundary::KoecherBruchhaeuser1<dim> >(
			std::stod(options.at(0)), // s
			std::stod(options.at(1))  // a
		);
		
		DTM::pout
			<< "dirichlet_boundary selector: created KoecherBruchhaeuser1 "
			<< "as dirichlet_boundary function, with " << std::endl
	...
	if (_type.compare("DirichletBoundary_KoecherBruchhaeuser2") == 0) {
		...
		function = std::make_shared< diffusion::dirichlet_boundary::KoecherBruchhaeuser2<dim> >(
			std::stod(options.at(0)), // s
			std::stod(options.at(1))  // a
		);
