...
// PROJECT includes
#include <diffusion/ErrorEstimator/ErrorEstimator_DWR__cGp_dG0__cGq_cG1.tpl.hh>
...
estimate(
	...
	std::shared_ptr< diffusion::Grid_DWR<dim,1> > _grid,
	std::shared_ptr< diffusion::dwr::ParameterSet > _parameter_set,
...
dual_get_z_t_on_slab(
	...
	if (parameter_set->fe.dual.time_type_support_points.compare("Gauss-Lobatto")==0) {
		...
		AssertThrow(
			false,
			dealii::ExcMessage("dual time type support points unknown")
		);
	}
