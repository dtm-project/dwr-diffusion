/**
 * @file Permeability_Selector.tpl.cc
 * @author Uwe Koecher (UK)
 * @author Marius Paul Bruchhäuser (MPB)
 *
 * @date 2018-07-30, Permeability_Selector, contributed by MPB from Force_Selector (UK)
...
create_function(
	...
	DTM::pout << "* found configuration: permeability function = " << _type << std::endl;
	DTM::pout << "* found configuration: permeability options = " << std::endl;
	...
	if (_type.compare("ZeroFunction") == 0) {
		...
		DTM::pout
			<< "permeability selector: created zero function" << std::endl
			<< std::endl;
	...
	if (_type.compare("ConstantFunction") == 0) {
		...
		DTM::pout
			<< "permeability selector: created ConstantFunction "
			<< "as permeability function, with " << std::endl
	...
	AssertThrow(
		false,
		dealii::ExcMessage("permeability function unknown, please check your input file data.")
	);
}

}} //namespaces

#include "Permeability_Selector.inst.in"
