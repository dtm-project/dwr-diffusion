...
#include <diffusion/ControlVolume/ControlVolumes.hh>
...
create_function(
	...
	if (_type.compare("ControlVolume_hyper_rectangle") == 0) {
		...
		function = std::make_shared< diffusion::control_volume::hyper_rectangle<dim> >(
			p1,p2,t1,t2
		);
	...
	if (_type.compare("ControlVolume_moving_hyper_rectangle") == 0) {
		...
		function = std::make_shared< diffusion::control_volume::moving_hyper_rectangle<dim> >(
			r1, p1,p2, t1,t2
		);
