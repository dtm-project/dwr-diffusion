...
#include <diffusion/ControlVolume/ControlVolume_Selector.tpl.hh>
...
template <int dim>
using NeumannAssembler = diffusion::Assemble::L2::NeumannConstrained::Assembler<dim>;
...
template<int dim>
void
Diffusion_DWR__cGp_dG0__cGq_cG1<dim>::
...
init_functions() {
	...
	{
		diffusion::neumann_boundary::Selector<dim> selector;
		selector.create_function(
	...
	{
		diffusion::control_volume::Selector<dim> selector;
		selector.create_function(
	...
	*primal.M = 0;
	{
		...
		DTM::pout << "dwr-diffusion: assemble mass matrix...";
	...
	*primal.A = 0;
	{
		...
		DTM::pout << "dwr-diffusion: assemble cell stiffness matrix...";
...
primal_assemble_rhs(
	...
	DTM::pout << "dwr-diffusion: assemble force f0...";
	...
	DTM::pout << "dwr-diffusion: assemble Neumann boundary terms u_N0...";
...
primal_solve_slab_problem(
	...
	DTM::pout << "dwr-diffusion: construct system matrix K = M + tau A...";
	...
	*primal.K = 0;
	...
	DTM::pout << "dwr-diffusion: construct linear system rhs vector...";
	...
	DTM::pout << "dwr-diffusion: dealii::MatrixTools::apply_boundary_values...";
	...
	DTM::pout << "dwr-diffusion: slab->primal.constraints->condense(*primal.K)...";
	...
	DTM::pout << "dwr-diffusion: setup direct lss and solve...";
	...
	DTM::pout << "dwr-diffusion: primal.constraints->distribute...";
...
dual_assemble_rhs(
	...
	auto assemble_Je = std::make_shared<
		diffusion::Assemble::L2::Je_global_L2L2::Assembler<dim> > (
		slab->dual.dof,
		slab->dual.fe,
		slab->dual.mapping,
		slab->dual.constraints
	...
	*u0_on_primal = 0;
	...
	// assemble J(v)(e) = (v,e)
	DTM::pout << "dwr-diffusion: assemble Je0...";
	...
	*dual.Je0 *= 1./primal_L2_L2_error_u;
	...
	if (parameter_set->fe.dual.time_type_support_points.compare("Gauss-Lobatto")==0) {
		...
		// assemble J(v)(e) = (v,e)
		DTM::pout << "dwr-diffusion: assemble Je1...";
...
dual_solve_slab_problem(
	...
	*dual.K = 0;
	...
	if (parameter_set->fe.dual.time_type_support_points.compare("Gauss-Lobatto")==0) {
		...
		DTM::pout << "dwr-diffusion: construct system matrix K = M + tau/2 A...";
		dual.K->add(slab->tau_n()/2., *dual.A);
		dual.K->add(1., *dual.M);
	}
	else if (parameter_set->fe.dual.time_type_support_points.compare("Gauss")==0) {
		// construct cG(1)-Q_G(2) system matrix K = 2 M + tau A
		DTM::pout << "dwr-diffusion: construct system matrix K = 2 M + tau A...";
	...
	if (parameter_set->fe.dual.time_type_support_points.compare("Gauss-Lobatto")==0) {
		...
		DTM::pout
			<< "dwr-diffusion: construct linear system rhs vector "
	...
	DTM::pout << "dwr-diffusion: slab->dual.constraints->condense(*dual.K)...";
	...
	DTM::pout << "dwr-diffusion: dual.constraints->distribute...";
...
compute_error_indicators() {
	error_estimator.dwr =
		std::make_shared< diffusion::dwr::cGp_dG0::cGq_cG1::ErrorEstimator<dim> > ();
...
#include "Diffusion_DWR__cGp_dG0__cGq_cG1.inst.in"
